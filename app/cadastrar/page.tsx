...
import { User, Mail, Lock, Eye, EyeOff, TrendingUp, BarChart2, Zap } from 'lucide-react'
    ...
    <div style={{display:'flex',minHeight:'100vh',background:'var(--background)',fontFamily:'Kanit,sans-serif'}}>
      ...
      <div style={{width:'480px',flexShrink:0,display:'flex',flexDirection:'column',justifyContent:'center',padding:'48px',borderLeft:'1px solid var(--border)'}}>
        <div style={{marginBottom:'40px'}}>
          <h2 style={{fontSize:'28px',fontWeight:800,color:'var(--foreground)',marginBottom:'8px'}}>Crie sua conta!</h2>
          <p style={{color:'rgba(255,255,255,0.4)',fontSize:'15px'}}>Comece a concorrer a premios hoje!</p>
        </div>
        {error && (
        ...
        <form onSubmit={handleCadastro} style={{display:'flex',flexDirection:'column',gap:'16px'}}>
          <div>
            <label style={{fontSize:'13px',color:'rgba(255,255,255,0.5)',display:'block',marginBottom:'6px'}}>Nome completo</label>
            <div style={{position:'relative'}}>
              <User style={{position:'absolute',left:'14px',top:'50%',transform:'translateY(-50%)',width:'16px',height:'16px',color:'rgba(255,255,255,0.3)'}}/>
              <input type="text" value={name} onChange={e=>setName(e.target.value)} required placeholder="Seu nome" style={{width:'100%',background:'var(--muted)',border:'1px solid var(--border)',borderRadius:'10px',padding:'14px 14px 14px 42px',color:'var(--foreground)',fontSize:'15px',outline:'none',fontFamily:'Kanit,sans-serif'}} onFocus={e=>e.target.style.borderColor='rgba(106,221,0,0.5)'} onBlur={e=>e.target.style.borderColor='var(--border)'}/>
            </div>
          </div>
          <div>
        ...
        <p style={{textAlign:'center',color:'rgba(255,255,255,0.4)',fontSize:'14px'}}>
          Ja tem uma conta? <Link href="/login" style={{color:'var(--primary)',fontWeight:600,textDecoration:'none'}}>Entrar</Link>
        </p>
      </div>
    </div>
  )
