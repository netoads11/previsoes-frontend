    ...
    <div style={{
        ...
        <div style={{padding:'0 20px'}}>
          ...
          <div style={{display:'flex',alignItems:'center',background:'var(--card)',border:'1px solid var(--border)',borderRadius:'10px',overflow:'hidden',transition:'border-color 0.15s, box-shadow 0.15s'}}>
            ...
            <input
              ...
              onFocus={e=>{const w=e.target.parentElement as HTMLElement;if(w){w.style.borderColor='var(--primary)';w.style.boxShadow='0 0 0 2px rgba(var(--primary-rgb, 0,230,118),0.15)'}}}
          ...
          <button disabled={num < min || loading} onClick={requestDeposit}
            ...
            onMouseLeave={e=>{if(num>=min)(e.currentTarget as HTMLButtonElement).style.background='var(--primary)'}}
          >
      ...
      {step === 'done' && (
        <div style={{padding:'40px 20px',textAlign:'center'}}>
          ...
          <p style={{fontSize:'14px',color:'var(--muted-foreground)',marginBottom:'8px'}}>R$ <span style={{color:'var(--primary)',fontWeight:700}}>{num.toFixed(2).replace('.',',')}</span> creditados na sua conta</p>
          <p style={{fontSize:'13px',color:'#444',marginBottom:'32px'}}>Novo saldo: <span style={{color:'var(--primary)',fontWeight:700}}>R$ {balance.toFixed(2).replace('.',',')}</span></p>
          <button onClick={onClose} style={{width:'100%',padding:'17px',borderRadius:'14px',border:'none',background:'var(--primary)',color:'#000',fontWeight:900,fontSize:'16px',cursor:'pointer',boxShadow:'0 0 28px rgba(var(--primary-rgb, 0,230,118),0.3)'}}>
            APOSTAR AGORA
          </button>
        </div>
      )}
